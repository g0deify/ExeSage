...
internal class SignatureAnalyzer {
    ...
    private void CheckCertificateDetails(ExecutableMetadata metadata)
    {
        try
        {
            var cert = new X509Certificate2(metadata.FilePath);
            metadata.SignerName = cert.GetNameInfo(X509NameType.SimpleName, false);
            metadata.CertificateIssuer = cert.GetNameInfo(X509NameType.SimpleName, true);

            // Self-signed: subject == issuer
            if (cert.Subject == cert.Issuer)
            {
                metadata.IsSelfSigned = true;
                metadata.PeAnomalies.Add(new PeAnomaly
                {
                    Type = "Certificate",
                    Description = "Self-signed certificate — no trusted CA verification",
                    Severity = "Medium"
                });
            }
