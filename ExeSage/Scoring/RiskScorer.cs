...
internal class RiskScorer {
    ...
    private double ScorePeAnomalies(ExecutableMetadata m, bool trusted) {
        ...
        if (grouped.ContainsKey("Resource")) score += 10.0;
        if (grouped.ContainsKey("VersionInfo")) score += 8.0;
        if (grouped.ContainsKey("EmbeddedPE")) score += 20.0;
        if (grouped.ContainsKey("RichHeader")) score += 10.0;
        if (grouped.ContainsKey("Certificate")) score += 10.0;
        return score;
    }
}
